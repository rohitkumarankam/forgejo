...
func TestPullRemoveAutomerge(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, u *url.URL) {
		...
		user5Session.MakeRequest(t, NewRequestWithValues(t, "POST", fmt.Sprintf("/%s/settings/branches/edit", repo.FullName()), map[string]string{
			"_csrf":              GetCSRF(t, user5Session, fmt.Sprintf("/%s/settings/branches/edit", repo.FullName())),
			"rule_name":          "main",
		...
		user5Session.MakeRequest(t, NewRequestWithValues(t, "POST", fmt.Sprintf("/%s/pulls/1/merge", repo.FullName()), map[string]string{
			"_csrf":                     GetCSRF(t, user5Session, fmt.Sprintf("/%s/pulls/1", repo.FullName())),
		...
		t.Run("No permission", func(t *testing.T) {
			...
			user2Session.MakeRequest(t, NewRequestWithValues(t, "POST", fmt.Sprintf("/%s/pulls/1/cancel_auto_merge", repo.FullName()), map[string]string{
				"_csrf": GetCSRF(t, user2Session, fmt.Sprintf("/%s/pulls/1", repo.FullName())),
			}), http.StatusSeeOther)
		...
		t.Run("Normal", func(t *testing.T) {
			...
			user5Session.MakeRequest(t, NewRequestWithValues(t, "POST", fmt.Sprintf("/%s/pulls/1/cancel_auto_merge", repo.FullName()), map[string]string{
				"_csrf": GetCSRF(t, user5Session, fmt.Sprintf("/%s/pulls/1", repo.FullName())),
			}), http.StatusSeeOther)
